...
两者不仅是并行输入，还存在交叉校验关系：
Figma 的结构信号 ←→ One-pager 的语义信号 = 互相验证 + 补全
```
...
┌─────────────────────────────────────────────────────────────────┐
│  Stage 0: Ingestion                                              │
│  Figma MCP / Export JSON + Frame Screenshots                    │
│  Product One-pager（业务语义信号）                                │
└────────────────────────────┬────────────────────────────────────┘
                             ↓
┌─────────────────────────────────────────────────────────────────┐
│  Stage 1: Figma Normalization Agent  [新增]                     │
│  • 命名语义化（模式匹配 + LLM 推断 + One-pager 交叉验证）       │
│  • Auto Layout 修复（Vision 补充）                               │
│  • Variants 碎片合并                                             │
│  • Token 反向推断                                                │
│  • 输出：Semantic IR + confidence map                           │
└────────────────────────────┬────────────────────────────────────┘
                             ↓
┌─────────────────────────────────────────────────────────────────┐
│  Stage 2: Confidence Gate  [新增]                               │
│  • high confidence → 自动通过                                    │
│  • low confidence  → 生成 clarification questions               │
│  • unknown         → fallback 到 generic component              │
└────────────────────────────┬────────────────────────────────────┘
                             ↓
┌─────────────────────────────────────────────────────────────────┐
│  Stage 3: Style-First Generation                                │
│                                                                  │
│  Semantic IR → Component Tree                                   │
│    组件映射、Props 结构、Variants 展开                            │
│         ↓                                                        │
│  Component Tree → Style Layer                                   │
│    CSS/Token、响应式、A11y                                        │
│         ↓                                                        │
│  Style Layer → Final Code                                       │
│    组合输出、import 整理、文件结构                                │
└────────────────────────────┬────────────────────────────────────┘
                             ↓
┌─────────────────────────────────────────────────────────────────┐
│  Stage 4: Self-Review Agent  [新增]                             │
│  • 对照 Semantic IR 验证生成代码的完整性                          │
│  • 检查 token 使用、组件覆盖率                                    │
│  • 生成 PR description + 标注不确定项                            │
└────────────────────────────┬────────────────────────────────────┘
                             ↓
┌─────────────────────────────────────────────────────────────────┐
│  Stage 5: Feedback Loop  [新增]                                  │
│  • PR merge 后分析 reviewer 的修改                                │
│  • 自动生成 Registry / Token 映射更新建议                         │
│  • 调整 confidence 阈值                                          │
│  → 回流至 Stage 1，持续改善 Normalization 质量                   │
└─────────────────────────────────────────────────────────────────┘
```
