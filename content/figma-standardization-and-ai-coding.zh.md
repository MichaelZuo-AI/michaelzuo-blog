...
![数据摄入路径与信号强度](/diagrams/figma-ingestion-paths.svg)
...
![标准化最低门槛](/diagrams/figma-minimum-viable-standard.svg)

## Part 2：Engineering 解法 — 改良 Pipeline 降低标准化依赖

### 2.1 问题根源重新定位

当前痛点的本质不是 Figma 不规范，而是 pipeline 把 Figma 的原始信号直接喂给 Claude Code，缺少了一个「信号清洗 + 语义提升」的中间层。

![改良前后的 Pipeline 对比](/diagrams/figma-before-after-pipeline.svg)

### 2.2 核心改良：插入 Semantic IR 层

引入一个 **Figma-agnostic 的中间表示（IR）**，将 Figma 的原始结构翻译成对 Claude Code 友好的语义描述，与 Figma 是否规范解耦。

![Semantic IR 结构](/diagrams/figma-semantic-ir.svg)
...
![Confidence Gate 决策流程](/diagrams/figma-confidence-gate.svg)
