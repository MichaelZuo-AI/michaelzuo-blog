...
date: "2026-03-18"
spoiler: "Figma Code Connect + 自建策略分层协同，组合所有好工具来最大化 AI Coding Pipeline 的输出质量。"
---
...
今天跟 Figma team 有一个 call，了解到了 [Code Connect](https://help.figma.com/hc/en-us/articles/23920389749655-Code-Connect)——本质上在组件映射这个维度做了同样的事，而且因为是 Figma-native 的，在这个点上有天然优势。

这意味着我们的工具箱里多了一个强力选项。**问题不是「用哪个替代哪个」，而是如何把所有好的工具和机制有效地组合起来，最大化 pipeline 的输出质量。**
...
1. **Component Registry + Code Connect 协同**。这是最大的变化。对于设计系统内的组件，Code Connect 提供 Figma-native 的映射，通过 MCP Server 直接到达 AI Agent；对于设计系统外的 ad-hoc 元素或 Code Connect 未覆盖的场景，自建 Registry 作为补充层继续发挥作用。两者不是替代关系，而是分层协同。
...
这是一个更高效的架构——**每个工具做它最擅长的事，组合起来覆盖全场景**。
...
**关键转变**：Code Connect 作为 Phase 0 是一个几乎纯配置的步骤，能快速建立组件映射的基础能力。自建策略从 Phase 1 开始，专注于 Code Connect 覆盖不到的维度，让整体投入更聚焦。

## 实际考量

在将 Code Connect 纳入 pipeline 之前，几个现实问题需要评估：

**Plan 限制**：Code Connect 仅对 Organization 和 Enterprise plan 开放。如果你的团队在 Professional 或 Starter plan 上，Code Connect 这一层暂时缺失——自建 Component Registry 继续承担组件映射职责，其余策略不受影响。
...
**维护模式变化**：Code Connect 的映射维护嵌入在 Figma 工作流中，Design System 团队在管理组件时同步维护映射，比独立维护 YAML 更自然。但这要求 Design System 团队接受这个额外职责，需要组织层面的对齐。如果组织推动困难，自建 Registry 由 Engineering 团队 own 反而更可控。
...
Figma Code Connect 验证了上一篇文章的核心判断——组件到代码的映射是解决 AI Coding 质量问题的关键。它为我们的工具箱增加了一个强力的原生选项。

修订后的策略核心是**有效组合所有可用工具**：

- **Code Connect 处理它最擅长的部分**——设计系统组件的精确映射，通过 MCP 直达 AI Agent
- **自建策略处理 Code Connect 覆盖不到的部分**——Token 标准化、布局推断、ad-hoc 元素识别、质量把关
- **两者分层协同**，在各自的优势领域发力，而非互相替代

核心信条不变：**不要求设计师改变工作方式。** 实现路径更丰富了——平台能力和自建工程各司其职，组合起来覆盖从设计系统组件到 ad-hoc 元素的完整频谱。目标始终是同一个：让 pipeline 在任意输入质量下都能产出可用的输出。
